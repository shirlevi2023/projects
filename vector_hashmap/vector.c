 ...
 * Function which receives a pointer to an element
 * of the type stored in the vector
 * vector and frees it.
 */
 ...
 * @param elem_cmp_func func which is used to
 * compare elements stored in the vector.
 ...
 * @return the index of the given value if it is
 * in the vector ([0, vector_size - 1]).
...
int realloc_up_if_needed(vector *vector);
int realloc_down_if_needed(vector *vector);
                     ...
                     vector_elem_free elem_free_func)
{
  ...
  new_vector->data = (void**)malloc((new_vector->capacity) * sizeof(void *));
...
int vector_push_back(vector *vector, const void *value)
{
  ...
  if (!realloc_up_if_needed(vector))
    {
      return 0;
    }
...
int vector_erase(vector *vector, size_t ind)
{
  ...
  double load_factor_after = (double )(vector->size - 1) / vector->capacity;
  if (load_factor_after < VECTOR_MIN_LOAD_FACTOR)
    {
      if (realloc_down_if_needed(vector) == 0)
        {
          return 0;
        }
    }
  void* elem = vector->data[ind];

  for (size_t i = ind + 1; i < vector->size; i++)
    {
      vector->data[i - 1] = vector->data[i];
    }
  vector->elem_free_func(&(elem));
  vector->size--;
      ...
      if (vector->size != 0)
        {
          for (size_t i = 0; i < size; i++)
            {
              vector_erase(vector, vector_find(vector,
...
/**
 * this funcs take a vector and realloc its capacity
 * @param vector
 * @return 1 if the realloc up success, and 0 else
 */
int realloc_up_if_needed(vector *vector)
{
  double load_factor_after = (double) (vector->size + 1) / (vector->capacity);
  if (load_factor_after > VECTOR_MAX_LOAD_FACTOR)
    {
      size_t new_capacity = vector->capacity * VECTOR_GROWTH_FACTOR;
      void **tmp = (void **) realloc (vector->data,
...
/**
 * this funcs take a vector and realloc its capacity down
 * @param vector
 * @return 1 if the realloc dowm success, and 0 else
 */
int realloc_down_if_needed(vector *vector)
{
    size_t new_capacity = (double)vector->capacity / VECTOR_GROWTH_FACTOR;
    void **tmp = (void **) realloc (vector->data,
                                    new_capacity * sizeof (void *));
    if (tmp == NULL)
      {
